...
public class PersonController {
	...
	public static void getActor(Context ctx, Map<String, Object> model) {
		...
		Person person = PersonDAO.getActorSelector(RequestUtil.getQueryActor(ctx), 0);
		if(person != null) {
			...
			if(creditRoll != null) {
				...
				for(int i = 0; i < creditRoll.size(); i++) {
					movies.add(ShowDAO.getShowSelector(null, creditRoll.get(i).getShowId()));
				}
		...
		//adds the Person to the model to be sent to the page
			...
			//if there is no actor found it send to an error page
		}else {
